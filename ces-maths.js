// =========================================================
// DONNÉES DU PROGRAMME (MATHS) - NE PAS TOUCHER
// =========================================================
var CHAPITRES = {
    '3e': [
        {
...
// Mise à jour des questions avec la géographie
for (var matiere in MATIERES){
    for (var annee in MATIERES[matiere]){
        var chaps = MATIERES[matiere][annee];
        for (var c=0; c<chaps.length; c++){
            if (chaps[c].exercices){
                for (var e=0; e<chaps[c].exercices.length; e++){
                    QUESTIONS_QUIZ.push({ id:'q'+QUESTIONS_QUIZ.length, matiere:matiere, annee:annee, chapitre:chaps[c].id, question:chaps[c].exercices[e].question, options:chaps[c].exercices[e].options, correct:chaps[c].exercices[e].correct, correction:chaps[c].exercices[e].correction || '' });
                }
            }
        }
    }
