...
function openChapitre(chapitreId) {
    ...
    var html = `
        ...
        <div style="margin: 16px 0; display: flex; gap: 10px; flex-wrap: wrap;">
            ...
            <button class="ghost-btn" onclick="document.body.removeChild(this.parentElement.parentElement.parentElement)">← Retour</button>
        </div>
    ...
    if (found.exercices) {
        ...
        for (var e = 0; e < found.exercices.length; e++) {
            ...
            for (var p = 0; p < ex.options.length; p++) {
                ...
                html += `
                    <button style="text-align: left; padding: 8px 12px; border: 1px solid var(--line); border-radius: 6px; background: var(--surface); cursor: pointer;" 
    ...
    var html = `
        <div style="text-align: center; padding: 30px 0;">
            ...
            <div style="display: flex; gap: 10px; justify-content: center; margin-top: 20px; flex-wrap: wrap;">
                ...
                <button class="ghost-btn" onclick="showTab('dashboard')">📊 Retour</button>
            </div>
        </div>
    `;
    container.innerHTML = html;
...
function finishExamen() {
    ...
    var html = `
        <div style="text-align: center; padding: 30px 0;">
            ...
            <p style="color: var(--muted);">${message}</p>
            <div style="display: flex; gap: 10px; justify-content: center; margin-top: 20px; flex-wrap: wrap;">
                <button class="primary-btn" onclick="startExamen('${currentExamen.questions[0]?.annee || '3e'}')">🔄 Refaire</button>
                <button class="ghost-btn" onclick="showTab('dashboard')">📊 Retour</button>
            </div>
        </div>
    `;
...
function renderRevisions() {
    ...
    if (total === 0) {
        html = '<div style="text-align: center; padding: 40px; color: var(--muted);">Aucun chapitre disponible.</div>';
    }
